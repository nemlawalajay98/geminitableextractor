        ...
        with open(text_path, 'rb') as f:
            ...
            if response.status_code == 200:
                ...
                if result.get('success') and result.get('extracted_data'):
                    ...
                    print("✅ Text file table extraction passed")
                    return True, result
                else:
                    print(f"❌ Text file table extraction failed: {result.get('error', 'Unknown error')}")
                    return False, result
            else:
                print(f"❌ Text file table extraction failed with status {response.status_code}")
                print(f"Response: {response.text}")
                return False, None
                
    except Exception as e:
        print(f"❌ Text file table extraction error: {str(e)}")
        return False, None
    finally:
        # Clean up
...
def main():
    ...
    export_data = image_data if image_success else (text_data if text_success else None)
